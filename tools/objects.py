...
def deepCompare(a, b, return_reason=False):
    ...
    if isinstance(a, (set, frozenset)):
        a = {
            e if e == e else math.nan for e in a
        }  # remplace les nan par des math.nan pour permetre comparaison
        b = {
            e if e == e else math.nan for e in b
        }  # remplace les nan par des math.nan pour permetre comparaison
    ...
    if hasattr(a, "__slots__"):
        for base_classe in a.__class__.__mro__[
            :-1
    ...
    if isinstance(a, dict):
        ...
        for key, value in a.items():
            if return_reason:
                same, raisonleft, raisonright = deepCompare(
                    value, b[key], return_reason=True
                )
                if not same:
                    return False, a.keys(), b.keys()
            else:
...
def ismethod_methoddescriptor_or_function(object):
    ...
    if isinstance(object, type):
        return False
    if isinstance(
        object, (types.MethodType, types.FunctionType, types.BuiltinFunctionType)
    ):
        return True
    tp = type(object)
    return hasattr(tp, "__get__") and not hasattr(tp, "__set__")
...
def add_Args(dictLocals):
    ...
    def __init__(self) :
        # sauve  init Arguments dans dictionnaire __initArgsDict__:
        saveInitArgsDict(locals())       
    def __getinitargs__(self) :
        # demande de restaurer avec __initArgsDict__ aussi bien avec Pickle que JSON:
        GenerateInitArgs(self)          
    def __getstate__(self)
        # filtre '__initArgsDict__' pour ne pas le sauver dans etat objet:
        return filtreInitArgsDict(self) 
    """
