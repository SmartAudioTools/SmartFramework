...
class SoftInputWidget(QtWidgets.QDialog):
    ...
    def update_panel_position(self):
        ...
        widget_bottom = widget.mapToGlobal(
            QtCore.QPoint(widget.frameGeometry().x(), widget.frameGeometry().y())
        ).y()
        screen_height = (
            QtWidgets.QApplication.instance().desktop().availableGeometry().height()
        )
        ...
        if (screen_height - widget_bottom) > input_panel_height:
            ...
            panelPos = QtCore.QPoint(
                widget_rect.left(), widget_rect.top() - input_panel_height
            )
...
class TouchInterface(QtCore.QObject):
    ...
    def eventFilter(self, widget, event):
        if (
            self._PARENT_WIDGET.focusWidget() == widget
