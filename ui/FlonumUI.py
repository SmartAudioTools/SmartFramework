...
class FlonumUI(QtWidgets.QDoubleSpinBox):
    def __init__(
        self,
        parent=None,
        value=0,
        ...
        **kwargs
        ...
        self._sync.output[float].connect(self.setValue)
    ...
    def setMaximum(self, max):
        QtWidgets.QDoubleSpinBox.setMaximum(self, max)

    # rajouté pour eviter que mon compilateur ne crée de property lui meme (car pas defini de get?)      :
    maximum = QtCore.Property(
        float, QtWidgets.QDoubleSpinBox.maximum, QtWidgets.QDoubleSpinBox.setMaximum
    )

    @QtCore.Slot(float)
    @QtCore.Slot(int)
    def setMinimum(self, max):
        QtWidgets.QDoubleSpinBox.setMinimum(self, max)

    # rajouté pour eviter que mon compilateur ne crée de property lui meme  (car pas defini de get?)          :
    minimum = QtCore.Property(
        float, QtWidgets.QDoubleSpinBox.minimum, QtWidgets.QDoubleSpinBox.setMinimum
    )

    @QtCore.Slot(str)
    def setPrefix(self, string):
        QtWidgets.QDoubleSpinBox.setPrefix(self, string)

    @QtCore.Slot(str)
    def setSuffix(self, string):
        QtWidgets.QDoubleSpinBox.setSuffix(self, string)
    ...
    def tabletEvent(self, event):
        if event.type() == QtCore.QEvent.TabletMove and event.pressure() > 0.0:
            ...
            if self._oldY is not None:
                ...
                self._floatValue -= deltaY * self._mouseSensibility * self.singleStep
                QtWidgets.QDoubleSpinBox.setValue(self, self._floatValue)
            self._oldY = newY
        ...
        if eventType == QtCore.QEvent.MouseMove:
            if event.buttons():
                ...
                if self._oldY is not None:
                    ...
                    deltaY = newY - self._oldY
                    self._floatValue -= (
                        deltaY * self._mouseSensibility * 10 ** (-self.decimals())
                    )
                    QtWidgets.QDoubleSpinBox.setValue(self, self._floatValue)
    ...
    def setValue(self, value):
        ...
        if value == QtWidgets.QDoubleSpinBox.value(self) and self._repeat:
            if not blockSignal:
                self.valueChanged.emit(value)
        else:
            if not blockSignal:
                QtWidgets.QDoubleSpinBox.setValue(self, value)
            else:
                self.blockSignals(True)
                QtWidgets.QDoubleSpinBox.setValue(self, value)
                self.blockSignals(False)

    value = QtCore.Property(float, QtWidgets.QDoubleSpinBox.value, setValue)
