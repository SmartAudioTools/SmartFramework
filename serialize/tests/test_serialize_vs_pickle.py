...
def addInFile(path, element, serializerName, encoding="utf_8_sig", newline="\n"):
...
def test_serialize_vs_pickle():
        ...
        for categoryName, categoryDict in objects.items():
            ...
            for key, value in categoryDict.items():
                ...
                try:
                    ...
                    continue
                    # break
                if serializerName != "serializejson_bytesIO":
                    addInFile(serializerDumpsPath, dumped, serializerName)
                else:
                    addInFile(serializerDumpsPath, bytesIO.read(), serializerName)
                if serializerName.startswith("serializejson"):
    ...
    # rapidjson:{get_hg_version('D:/Projets/Python/serializejson')},smartframework:{get_hg_version('D:/Projets/Python/SmartFramework')}":
    result_name = f"{datetime.now().strftime('%Y-%m-%d,%Hh%Mm%Ss')}"
    times = {"dumps": dumps_times_by_type, "loads": loads_times_by_type}

    write(
        directory(__file__) + f"/serialized/{result_name}_times.json",
        serializejson.dumps(times, sort_keys=False, indent=None),
    )
