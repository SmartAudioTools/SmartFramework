try:
    ...
    import sys
    import blosc
    from pybase64 import b64encode, b64decode_as_bytearray
    from rapidjson import RawBytesToPutInQuotes
    ...
    def serializejson_ndarray(inst):
        ...
        if dtype.fields is None:
            ...
            # return ("numpy.array", (RawString(numpy.array2string(inst,separator =',')), dtype_str), None)  plus lent.
        ...
        if serialize_parameters.numpy_array_use_numpyB64:
            if dtype == bool:
                ...
                elif serialize_parameters.use_tuple_for_numpy_shape:
                ...
                if len(compressed) < data.nbytes:
                    if len_or_shape is None:
                        return (
                            "numpyB64",
                            (
                                RawBytesToPutInQuotes(b64encode(compressed)),
                                dtype_str,
                                compression,
                            ),
                            None,
            ...
            if len_or_shape is None:
                return (
                    "numpyB64",
                    (
                        RawBytesToPutInQuotes(b64encode(numpy.ascontiguousarray(data))),
                        dtype_str,
                    ),
                    None,
                )
