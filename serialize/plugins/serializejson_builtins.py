...
import blosc
import types
from pybase64 import b64decode, b64decode_as_bytearray, b64encode
...
def serializejson_bytearray(inst):
    ...
    if (
        ...
        if len(compressed) < len(inst):
            return (
                "bytearray",
                (RawBytesToPutInQuotes(b64encode(compressed)), "b64_blosc"),
                None,
            )
    return "bytearray", (RawBytesToPutInQuotes(b64encode(inst)), "b64"), None
...
def serializejson_bytes(inst):
    ...
    return (
        ...
        None,
        None,
        (RawBytesToPutInQuotes(b64encode(inst)), "b64"),
    )
