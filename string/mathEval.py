...
class RunnableProcessing(multiprocessing.Process):
    ...
    def __init__(self, func, *args, **kwargs):
        ...
        multiprocessing.Process.__init__(
            self, target=self.run_func, args=args, kwargs=kwargs
        )
...
class BINARY_POWER(BINARY):
    ...
    def touch_value(self, stack, frame):
        ...
        print(TOS1, TOS)
        if (
            abs(TOS1) > BadValue.MAX_ALLOWED_VALUE
            or abs(TOS) > BadValue.MAX_ALLOWED_VALUE
        ):
...
class CALL_FUNCTION(OpcodeArg):
    ...
    def touch_value(self, stack, frame):
        ...
        assert (
            func in frame.values()
        ), "Uh-oh somebody injected bad function. This does not happen."
...
if __name__ == "__main__":
    ...
    try:
        safe_eval_timeout(
            "2**999999999999999999999**9999999999", check_compiling_input=False
        )
        raise AssertionError("Should not be reached")
