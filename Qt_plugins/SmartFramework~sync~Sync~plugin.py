...
icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Sync.png")
if os.path.exists(icon_path):
    Sync.__icon__ = QtGui.QIcon(icon_path)
else:
    Sync.__icon__ = QtGui.QIcon()


class SyncPlugin(QtDesigner.QPyDesignerCustomWidgetPlugin):
    # The __init__() method is only used to set up the plugin and define its initialized variable.
    def __init__(self, parent=None):
        super(SyncPlugin, self).__init__(parent)
    ...
    def createWidget(self, parent):
        ...
        old_class = SmartFramework.sync.Sync.Sync
        SmartFramework.sync.Sync.Sync = Sync
        instance = Sync(parent=parent)
        SmartFramework.sync.Sync.Sync = old_class
        return instance
    ...
    def includeFile(self):
        return "SmartFramework.sync.Sync"
