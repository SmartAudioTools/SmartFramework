import os
from PySide6.QtDesigner import QPyDesignerCustomWidgetCollection
from SmartFramework.ui import exceptionDialog
import SmartFramework.time.TimeMonitorUI
from SmartFramework.designer import transformClassForQtDesigner


DOM_XML = """
<ui language='c++'>
    <widget class='TimeMonitorUI' name='TimeMonitorUI'>
    </widget>
</ui>
"""

icon = os.path.splitext(SmartFramework.time.TimeMonitorUI.__file__)[0] + ".png"
if not os.path.exists(icon):
    icon = "/home/smartaudiotools/icones/Blank.png"

# hack pour permetre de rajouter interface graphique à un objet non graphique
TimeMonitorUI = transformClassForQtDesigner(
    SmartFramework.time.TimeMonitorUI.TimeMonitorUI
)


def new_super(class_, inst, patched_class=TimeMonitorUI):
    if isinstance(inst, patched_class):
        return super(patched_class, inst)
    else:
        return super(class_, inst)


SmartFramework.time.TimeMonitorUI.super = new_super


# contourne bug de PySide6: https://bugreports.qt.io/browse/PYSIDE-1884
# tool_tip = TimeMonitorUI.__doc__ # pourra remetre quand bug corrigé
tool_tip = TimeMonitorUI.__new__(TimeMonitorUI).__doc__
if tool_tip is None:
    tool_tip = ""


QPyDesignerCustomWidgetCollection.registerCustomWidget(
    TimeMonitorUI,
    module="TimeMonitorUI",
    tool_tip=tool_tip,
    xml=DOM_XML,
    icon=icon,
    group="SmartFramework.time",
    container=False,
)
