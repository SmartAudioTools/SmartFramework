import os
import SmartFramework.audio.AudioDeviceMenuUI
...
AudioDeviceMenuUI = transformClassForQtDesigner(
    SmartFramework.audio.AudioDeviceMenuUI.AudioDeviceMenuUI
)
icon_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "AudioDeviceMenuUI.png"
)
if os.path.exists(icon_path):
    AudioDeviceMenuUI.__icon__ = QtGui.QIcon(icon_path)
else:
    AudioDeviceMenuUI.__icon__ = QtGui.QIcon()


class AudioDeviceMenuUIPlugin(QtDesigner.QPyDesignerCustomWidgetPlugin):
    # The __init__() method is only used to set up the plugin and define its initialized variable.
    def __init__(self, parent=None):
        super(AudioDeviceMenuUIPlugin, self).__init__(parent)
    ...
    def createWidget(self, parent):
        ...
        old_class = SmartFramework.audio.AudioDeviceMenuUI.AudioDeviceMenuUI
        SmartFramework.audio.AudioDeviceMenuUI.AudioDeviceMenuUI = AudioDeviceMenuUI
        instance = AudioDeviceMenuUI(parent=parent)
        SmartFramework.audio.AudioDeviceMenuUI.AudioDeviceMenuUI = old_class
        return instance

    # This method returns the name of the custom widget class that is provided by this plugin.
    def name(self):
        return "AudioDeviceMenuUI"

    # Returns the name of the group in Qt Designer's widget box that this widget belongs to.
    def group(self):
        return "SmartFramework.audio"

    # Returns the icon used to represent the custom widget in Qt Designer's widget box.
    def icon(self):
        return AudioDeviceMenuUI.__icon__

    # Returns a short description of the custom widget for use in a tool tip.
    def toolTip(self):
        return AudioDeviceMenuUI.__doc__
    ...
    def includeFile(self):
        return "SmartFramework.audio.AudioDeviceMenuUI"
