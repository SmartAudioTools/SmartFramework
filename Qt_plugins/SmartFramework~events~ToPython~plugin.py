import os
import SmartFramework.events.ToPython
...
ToPython = transformClassForQtDesigner(SmartFramework.events.ToPython.ToPython)
icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ToPython.png")
if os.path.exists(icon_path):
    ToPython.__icon__ = QtGui.QIcon(icon_path)
else:
    ToPython.__icon__ = QtGui.QIcon()


class ToPythonPlugin(QtDesigner.QPyDesignerCustomWidgetPlugin):
    # The __init__() method is only used to set up the plugin and define its initialized variable.
    def __init__(self, parent=None):
        super(ToPythonPlugin, self).__init__(parent)
    ...
    def createWidget(self, parent):
        ...
        old_class = SmartFramework.events.ToPython.ToPython
        SmartFramework.events.ToPython.ToPython = ToPython
        instance = ToPython(parent=parent)
        SmartFramework.events.ToPython.ToPython = old_class
        return instance
    ...
    def icon(self):
        return ToPython.__icon__

    # Returns a short description of the custom widget for use in a tool tip.
    def toolTip(self):
        return ToPython.__doc__
    ...
    def includeFile(self):
        return "SmartFramework.events.ToPython"
