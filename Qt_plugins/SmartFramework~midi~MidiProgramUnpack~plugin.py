import os
import SmartFramework.midi.MidiProgramUnpack
...
MidiProgramUnpack = transformClassForQtDesigner(
    SmartFramework.midi.MidiProgramUnpack.MidiProgramUnpack
)
icon_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "MidiProgramUnpack.png"
)
if os.path.exists(icon_path):
    MidiProgramUnpack.__icon__ = QtGui.QIcon(icon_path)
else:
    MidiProgramUnpack.__icon__ = QtGui.QIcon()


class MidiProgramUnpackPlugin(QtDesigner.QPyDesignerCustomWidgetPlugin):
    # The __init__() method is only used to set up the plugin and define its initialized variable.
    def __init__(self, parent=None):
        super(MidiProgramUnpackPlugin, self).__init__(parent)
    ...
    def createWidget(self, parent):
        ...
        old_class = SmartFramework.midi.MidiProgramUnpack.MidiProgramUnpack
        SmartFramework.midi.MidiProgramUnpack.MidiProgramUnpack = MidiProgramUnpack
        instance = MidiProgramUnpack(parent=parent)
        SmartFramework.midi.MidiProgramUnpack.MidiProgramUnpack = old_class
        return instance

    # This method returns the name of the custom widget class that is provided by this plugin.
    def name(self):
        return "MidiProgramUnpack"
    ...
    def icon(self):
        return MidiProgramUnpack.__icon__

    # Returns a short description of the custom widget for use in a tool tip.
    def toolTip(self):
        return MidiProgramUnpack.__doc__
    ...
    def includeFile(self):
        return "SmartFramework.midi.MidiProgramUnpack"
