...
icon_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "VideoIndiceGenerator.png"
)
if os.path.exists(icon_path):
    VideoIndiceGenerator.__icon__ = QtGui.QIcon(icon_path)
else:
    VideoIndiceGenerator.__icon__ = QtGui.QIcon()


class VideoIndiceGeneratorPlugin(QtDesigner.QPyDesignerCustomWidgetPlugin):
    # The __init__() method is only used to set up the plugin and define its initialized variable.
    def __init__(self, parent=None):
        super(VideoIndiceGeneratorPlugin, self).__init__(parent)
    ...
    def createWidget(self, parent):
        ...
        old_class = SmartFramework.video.VideoIndiceGenerator.VideoIndiceGenerator
        SmartFramework.video.VideoIndiceGenerator.VideoIndiceGenerator = (
            VideoIndiceGenerator
        )
        instance = VideoIndiceGenerator(parent=parent)
        SmartFramework.video.VideoIndiceGenerator.VideoIndiceGenerator = old_class
        return instance

    # This method returns the name of the custom widget class that is provided by this plugin.
    def name(self):
        return "VideoIndiceGenerator"
    ...
    def icon(self):
        return VideoIndiceGenerator.__icon__

    # Returns a short description of the custom widget for use in a tool tip.
    def toolTip(self):
        return VideoIndiceGenerator.__doc__
    ...
    def includeFile(self):
        return "SmartFramework.video.VideoIndiceGenerator"
