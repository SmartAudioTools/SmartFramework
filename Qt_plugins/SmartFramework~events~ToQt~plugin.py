...
icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ToQt.png")
if os.path.exists(icon_path):
    ToQt.__icon__ = QtGui.QIcon(icon_path)
else:
    ToQt.__icon__ = QtGui.QIcon()


class ToQtPlugin(QtDesigner.QPyDesignerCustomWidgetPlugin):
    # The __init__() method is only used to set up the plugin and define its initialized variable.
    def __init__(self, parent=None):
        super(ToQtPlugin, self).__init__(parent)
    ...
    def createWidget(self, parent):
        ...
        old_class = SmartFramework.events.ToQt.ToQt
        SmartFramework.events.ToQt.ToQt = ToQt
        instance = ToQt(parent=parent)
        SmartFramework.events.ToQt.ToQt = old_class
        return instance
    ...
    def includeFile(self):
        return "SmartFramework.events.ToQt"
