...
icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Fps.png")
if os.path.exists(icon_path):
    Fps.__icon__ = QtGui.QIcon(icon_path)
else:
    Fps.__icon__ = QtGui.QIcon()


class FpsPlugin(QtDesigner.QPyDesignerCustomWidgetPlugin):
    # The __init__() method is only used to set up the plugin and define its initialized variable.
    def __init__(self, parent=None):
        super(FpsPlugin, self).__init__(parent)
    ...
    def createWidget(self, parent):
        ...
        old_class = SmartFramework.video.Fps.Fps
        SmartFramework.video.Fps.Fps = Fps
        instance = Fps(parent=parent)
        SmartFramework.video.Fps.Fps = old_class
        return instance
    ...
    def includeFile(self):
        return "SmartFramework.video.Fps"
