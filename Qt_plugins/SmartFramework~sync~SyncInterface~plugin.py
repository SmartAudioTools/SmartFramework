import os
import SmartFramework.sync.SyncInterface
...
SyncInterface = transformClassForQtDesigner(
    SmartFramework.sync.SyncInterface.SyncInterface
)
icon_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "SyncInterface.png"
)
if os.path.exists(icon_path):
    SyncInterface.__icon__ = QtGui.QIcon(icon_path)
else:
    SyncInterface.__icon__ = QtGui.QIcon()


class SyncInterfacePlugin(QtDesigner.QPyDesignerCustomWidgetPlugin):
    # The __init__() method is only used to set up the plugin and define its initialized variable.
    def __init__(self, parent=None):
        super(SyncInterfacePlugin, self).__init__(parent)
    ...
    def createWidget(self, parent):
        ...
        old_class = SmartFramework.sync.SyncInterface.SyncInterface
        SmartFramework.sync.SyncInterface.SyncInterface = SyncInterface
        instance = SyncInterface(parent=parent)
        SmartFramework.sync.SyncInterface.SyncInterface = old_class
        return instance
    ...
    def icon(self):
        return SyncInterface.__icon__

    # Returns a short description of the custom widget for use in a tool tip.
    def toolTip(self):
        return SyncInterface.__doc__
    ...
    def includeFile(self):
        return "SmartFramework.sync.SyncInterface"
