import os
import SmartFramework.midi.MidiChannelSplit
...
MidiChannelSplit = transformClassForQtDesigner(
    SmartFramework.midi.MidiChannelSplit.MidiChannelSplit
)
icon_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "MidiChannelSplit.png"
)
if os.path.exists(icon_path):
    MidiChannelSplit.__icon__ = QtGui.QIcon(icon_path)
else:
    MidiChannelSplit.__icon__ = QtGui.QIcon()


class MidiChannelSplitPlugin(QtDesigner.QPyDesignerCustomWidgetPlugin):
    # The __init__() method is only used to set up the plugin and define its initialized variable.
    def __init__(self, parent=None):
        super(MidiChannelSplitPlugin, self).__init__(parent)
    ...
    def createWidget(self, parent):
        ...
        old_class = SmartFramework.midi.MidiChannelSplit.MidiChannelSplit
        SmartFramework.midi.MidiChannelSplit.MidiChannelSplit = MidiChannelSplit
        instance = MidiChannelSplit(parent=parent)
        SmartFramework.midi.MidiChannelSplit.MidiChannelSplit = old_class
        return instance

    # This method returns the name of the custom widget class that is provided by this plugin.
    def name(self):
        return "MidiChannelSplit"
    ...
    def icon(self):
        return MidiChannelSplit.__icon__

    # Returns a short description of the custom widget for use in a tool tip.
    def toolTip(self):
        return MidiChannelSplit.__doc__
    ...
    def includeFile(self):
        return "SmartFramework.midi.MidiChannelSplit"
