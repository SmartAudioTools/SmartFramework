...
icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Gate.png")
if os.path.exists(icon_path):
    Gate.__icon__ = QtGui.QIcon(icon_path)
else:
    Gate.__icon__ = QtGui.QIcon()


class GatePlugin(QtDesigner.QPyDesignerCustomWidgetPlugin):
    # The __init__() method is only used to set up the plugin and define its initialized variable.
    def __init__(self, parent=None):
        super(GatePlugin, self).__init__(parent)
    ...
    def createWidget(self, parent):
        ...
        old_class = SmartFramework.events.Gate.Gate
        SmartFramework.events.Gate.Gate = Gate
        instance = Gate(parent=parent)
        SmartFramework.events.Gate.Gate = old_class
        return instance
    ...
    def includeFile(self):
        return "SmartFramework.events.Gate"
