import os
import SmartFramework.video.Webcam
...
Webcam = transformClassForQtDesigner(SmartFramework.video.Webcam.Webcam)
icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Webcam.png")
if os.path.exists(icon_path):
    Webcam.__icon__ = QtGui.QIcon(icon_path)
else:
    Webcam.__icon__ = QtGui.QIcon()


class WebcamPlugin(QtDesigner.QPyDesignerCustomWidgetPlugin):
    # The __init__() method is only used to set up the plugin and define its initialized variable.
    def __init__(self, parent=None):
        super(WebcamPlugin, self).__init__(parent)
    ...
    def createWidget(self, parent):
        ...
        old_class = SmartFramework.video.Webcam.Webcam
        SmartFramework.video.Webcam.Webcam = Webcam
        instance = Webcam(parent=parent)
        SmartFramework.video.Webcam.Webcam = old_class
        return instance
    ...
    def icon(self):
        return Webcam.__icon__

    # Returns a short description of the custom widget for use in a tool tip.
    def toolTip(self):
        return Webcam.__doc__
    ...
    def includeFile(self):
        return "SmartFramework.video.Webcam"
