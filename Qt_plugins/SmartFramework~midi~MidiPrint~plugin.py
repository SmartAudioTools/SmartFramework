import os
import SmartFramework.midi.MidiPrint
...
MidiPrint = transformClassForQtDesigner(SmartFramework.midi.MidiPrint.MidiPrint)
icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "MidiPrint.png")
if os.path.exists(icon_path):
    MidiPrint.__icon__ = QtGui.QIcon(icon_path)
else:
    MidiPrint.__icon__ = QtGui.QIcon()


class MidiPrintPlugin(QtDesigner.QPyDesignerCustomWidgetPlugin):
    # The __init__() method is only used to set up the plugin and define its initialized variable.
    def __init__(self, parent=None):
        super(MidiPrintPlugin, self).__init__(parent)
    ...
    def createWidget(self, parent):
        ...
        old_class = SmartFramework.midi.MidiPrint.MidiPrint
        SmartFramework.midi.MidiPrint.MidiPrint = MidiPrint
        instance = MidiPrint(parent=parent)
        SmartFramework.midi.MidiPrint.MidiPrint = old_class
        return instance
    ...
    def icon(self):
        return MidiPrint.__icon__

    # Returns a short description of the custom widget for use in a tool tip.
    def toolTip(self):
        return MidiPrint.__doc__
    ...
    def includeFile(self):
        return "SmartFramework.midi.MidiPrint"
