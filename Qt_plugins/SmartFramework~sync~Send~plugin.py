...
icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Send.png")
if os.path.exists(icon_path):
    Send.__icon__ = QtGui.QIcon(icon_path)
else:
    Send.__icon__ = QtGui.QIcon()


class SendPlugin(QtDesigner.QPyDesignerCustomWidgetPlugin):
    # The __init__() method is only used to set up the plugin and define its initialized variable.
    def __init__(self, parent=None):
        super(SendPlugin, self).__init__(parent)
    ...
    def createWidget(self, parent):
        ...
        old_class = SmartFramework.sync.Send.Send
        SmartFramework.sync.Send.Send = Send
        instance = Send(parent=parent)
        SmartFramework.sync.Send.Send = old_class
        return instance
    ...
    def includeFile(self):
        return "SmartFramework.sync.Send"
